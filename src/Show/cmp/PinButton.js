...
export default function PinButton(props) {
  ...
  if (props.loading) {
    ...
    return <button onClick={() => { props.pinPost({post_id: props.post_id, user_id: props.user_id, post_title: props.post_title, token: props.token}) }} className='btn btn-primary'><i className="fas fa-map-pin"></i> Pin</button>
  }
}